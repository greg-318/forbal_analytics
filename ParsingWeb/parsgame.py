...
def links(page):
    ...
    games = tuple(x.get_attribute("href") for x in quantity if "match" in str(x.get_attribute("href")))
    if not games:
        return
    return games
...
def game(http):
    ...
    for item, ti in zip(table, indicator.dict().keys()):
        if 'CHANCES' in str(item.find_all('div', class_='progress-title')):
            data = tuple(x.get('title')[:-1] for x in item.find_all('div')if x.get('title'))
        else:
...
def players(url):
    ...
    :return: Возвращает два списка с теми игроками которые играли в данном матче одний команды и второй .
    """
    driver.get(url)
    html_players = driver.page_source
    soup = BeautifulSoup(html_players, 'html.parser')
    table = soup.find('tbody')
    team_1 = sum([[y.text for i, y in enumerate(x) if i == 1] for x in table.find_all('tr') if "Sub" not in str(x)], [])
    ...
    team_2 = sum([[y.text for i, y in enumerate(x) if i == 1] for x in table1.find_all('tr') if "Sub" not in str(x)], [])
...
def insert( draw, team1, team2):
    ...
    with MongoDefault('gameIndicators') as md:
        ...
        if not response:
            logging.warning(f'Something happened during insertion {match} into the database')
            return False
        else:
            return response
        ...
        for ligue in ligues:
            ...
            for year in years:
                for team in teams:
                    ...
                    if not links:
                        logging.warning(f"not possible to find team matches{http}")
                        continue
                    for link in links_page:
                        ...
                        print(status)
                        if not status:
                            logging.warning(f'Something happened during insertion {link} into the database')
                            continue
        time.sleep(345600)
